...
- **A session is no longer closed out from under a request that just picked it up.** When a session outlived its lifetime, the check for whether anything was using it and the replacement that followed were separate steps, so a request arriving in between could have its browser closed mid-solve and fail with an error it could do nothing about. Both engines were affected, and the stealth engine did not lock the check at all.
